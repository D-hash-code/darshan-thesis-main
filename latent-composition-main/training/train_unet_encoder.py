...
def train(opt):
    ...
    This Model is the encoder that takes a cropped image and mask and returns the latent space input for our CNF
    We use the U-Net as it allows us to go from Image to Noise (Although will need to check if we can have multiple channels)


    1. Set pre run things; seeds, args
    ...
    3. Create Encoder
    3.1 Set Optimiser
    3.1.1 Hyperparameters: Learning rate, optimiser type
    3.1.2 Define Losses (MSE, LPIPS)
    3.2 Set Input, output dimensions (c,w,h)

    4. Configure Train and Test Loader (Datasets)
    The original latent encoder trains by; sampling noise, generating images, running generated images through
    encoder, running encoded noise through generator.
    Our CNF has the capability of training on real data too. Simply we backward run the real image through CNF to get noise.

    The data that the model trains on is either real data or generated noise fed into the network.
    
    Maybe the train loader can just be of the real data? And we generate and apply the masks later? Is this costly?
    4.1 

    5. Training Computations
    
    5.1 Get Fake Images
    5.2 Get Encoded Images
    5.3 Get Regenerated Images

    6. Loss Computation

    6.1 Z loss = Correlation_Square_Error_Loss(Z_data,Encoded)
    6.2 MSE Loss = MSE (regenerated, fake image)
    6.3 LPIPS Loss =  perceptual_loss.forward(reshaped(regenerated),reshaped(fake image))
    6.4 loss summation, loss.backward, optimiser.update

    7. Logging and Test Loader
    ...
    for epoch, epoch_loader in enumerate(pbar(training_utils.epoch_grouper(train_loader, epoch_batches),total=(opt.niter-start_ep)), start_ep):
        ...
        for step, z_batch in enumerate(pbar(epoch_loader, total=epoch_batches), 1):
            z_batch = z_batch.to(device)
            netE.zero_grad()
            fake_im = netG(z_batch).detach()
            if has_masked_input:
                hints_fake, mask_fake = masking.mask_upsample(fake_im)  ## hints fake is masked image
                encoded = netE(torch.cat([hints_fake, mask_fake], dim=1))
                if opt.masked:
                    regenerated = netG(encoded)
                elif opt.vae_like:    ## This is interesting! Not sure what they are doing here but its not in the paper
            ...
            loss_perceptual = perceptual_loss.forward(reshape(regenerated), reshape(fake_im)).mean()
            loss = (opt.lambda_latent * loss_z + opt.lambda_mse * loss_mse + opt.lambda_lpips * loss_perceptual)
