...
## Functionality to return x = CNF(z) etc...

def noise2img(model,z):
    
    #x=CNF(z)
    x, _, _ = model(z, reverse=True)

    return x, z


def img2noise(model,x):
...
## Training Regressor
    ## I could just mask it and propogate it back !! No need to train a seperate model...

    ## Ahh but the mask is also fed to this model so might need to use this model (Why and how is this mask given to this model?)
